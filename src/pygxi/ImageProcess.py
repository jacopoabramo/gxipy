...
class ImageProcess:
    ...
    def image_improvement(self, image, output_address, image_process_config):
        ...
        if not isinstance(image_process_config, ImageProcessConfig):
            raise ParameterTypeError(
                "image_process_config param must be ImageProcessConfig type."
            )
        ...
        if isinstance(image, (RawImage, RGBImage)):
            ...
            raise ParameterTypeError(
                "image param must be RawImage or GxImageInfo type."
            )
        ...
        if pixel_format in (GxPixelFormatEntry.RGB8, GxPixelFormatEntry.BGR8):
            if pixel_format == GxPixelFormatEntry.BGR8:
                ...
                status = dx.dx_image_format_convert_set_output_pixel_format(
                    self.image_convert_handle, GxPixelFormatEntry.RGB8
                )
                ...
                status = dx.dx_image_format_convert_set_valid_bits(
                    self.image_convert_handle, DxValidBit.BIT0_7
                )
                if status != dx.DxStatus.OK:
                    raise UnexpectedError(
                ...
                if status != dx.DxStatus.OK:
                    raise UnexpectedError(
                        "dx_image_format_convert_get_buffer_size_for_conversion failure, Error code:%s"
                        % hex(status).__str__()
                    )

                image_temp = (ct.c_ubyte * out_lenght)()
                output_image_temp = ct.addressof(image_temp)
                ...
                if status != dx.DxStatus.OK:
                    raise UnexpectedError(
                        "image_format_convert failure, Error code:%s"
                        % hex(status).__str__()
                    )

                input_image_buffer = output_image_temp
            status = dx.dx_image_improvement_ex(
                input_image_buffer,
                output_address,
                raw_image_width,
                raw_image_height,
        ...
        if (pixel_format & PIXEL_BIT_MASK) != GX_PIXEL_8BIT:
            ...
            status = dx.dx_image_format_convert_set_valid_bits(
                self.image_convert_handle, image_process_config.get_valid_bits()
            )
            if status != dx.DxStatus.OK:
                raise UnexpectedError(
                    "dx.dx_image_format_convert_set_output_pixel_format failure, Error code:%s"
            ...
            image_temp = (ct.c_ubyte * out_lenght)()
            output_8bit_image = ct.addressof(image_temp)
        ...
        if Utility.is_gray(pixel_format):
            ImageProcess.__mono_image_process(
                self, output_address, image, image_process_config
            )
        else:
            rgb_image_array_temp = (
                ct.c_ubyte * image.frame_data.height * image.frame_data.width * 3
            )()
            rgb_image_array_temp_address = ct.addressof(rgb_image_array_temp)
            ImageProcess.__raw_image_process(
                rgb_image_array_temp_address, image, image_process_config
            )
    ...
    def static_defect_correction(
        ...
        :return: status                            State return value, See detail in dx.DxStatus
        ...
        if not isinstance(defect_correction, dx.StaticDefectCorrection):
            raise ParameterTypeError(
                "StaticDefectCorrection param must be StaticDefectCorrection type."
            )

        if defect_pos_buffer_address is None:
            raise ParameterTypeError("defect_pos_buffer_address param is null pointer.")

        if not isinstance(defect_pos_buffer_size, int):
            raise ParameterTypeError("defect_pos_buffer_size param must be Int type.")

        status = dx.dx_static_defect_correction(
            input_address,
            output_address,
            defect_correction,
            defect_pos_buffer_address,
        ...
        if status != dx.DxStatus.OK:
            raise UnexpectedError(
                "dx_static_defect_correction failure, Error code:%s"
                % hex(status).__str__()
            )

    @staticmethod
    def calcula_lut(contrast_param, gamma, light_ness, lut_address, lut_length_address):
        """
        ...
        :return: status                            State return value, See detail in dx.DxStatus
                 data_array                        Array of output images, buff size = width * height * 3
        """
        if not (isinstance(contrast_param, int)):
        ...
        if not (isinstance(light_ness, int)):
        ...
        status = dx.dx_calc_camera_lut_buffer(
            contrast_param, gamma, light_ness, lut_address, lut_length_address
        )
        if status != dx.DxStatus.OK:
            raise UnexpectedError(
                "calc_camera_lut_buffer failure, Error code:%s" % hex(status).__str__()
            )
    ...
    def read_lut_file(lut_file_path, lut_address, lut_length_address):
        ...
        path = ct.create_string_buffer(dx.string_encoding(lut_file_path))

        status = dx.dx_read_lut_file(path, lut_address, lut_length_address)
        if status != dx.DxStatus.OK:
            raise UnexpectedError(
                "read_lut_file failure, Error code:%s" % hex(status).__str__()
            )
    ...
    def __get_pixel_color_filter(pixel_format):
        ...
        if pixel_format in gr_tup:
            return dx.DxPixelColorFilter.GR
    ...
    def __raw_image_process(output_address, image, image_process_config):
        ...
        if input_image_buffer is None or output_address is None:
            raise ParameterTypeError(
                "input_image_buffer or output_address is NULL pointer"
            )

        color_filter = ImageProcess.__get_pixel_color_filter(pixel_format)
        mutex = image_process_config.get_mutex()
        with mutex:
            color_img_process_param = image_process_config.get_color_image_process(
                color_filter
            )
            status = dx.dx_raw8_image_process(
                input_image_buffer,
                output_address,
                raw_image_width,
                raw_image_height,
            ...
            if status != dx.DxStatus.OK:
                raise UnexpectedError(
                    "RawImage.raw8_image_process: failed, error code:%s"
                    % hex(status).__str__()
                )
    ...
    def __mono_image_process(self, output_address, image, image_process_config):
        ...
        with mutex:
            ...
            status = dx.dx_mono8_image_process(
                input_image_buffer,
                output_address,
                raw_image_width,
                raw_image_height,
                mono_img_process_param,
            )
            if status != dx.DxStatus.OK:
                raise UnexpectedError(
                    "RawImage.dx_mono8_image_process: failed, error code:%s"
                    % hex(status).__str__()
                )
    ...
    def __check_handle(self):
        ...
        if self.image_convert_handle is None:
            ...
            if status != dx.DxStatus.OK:
                raise UnexpectedError(
                    "dx_image_format_convert_create failure, Error code:%s"
                    % hex(status).__str__()
                )
